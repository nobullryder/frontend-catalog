...
// External local import(s) that remain separate
import { Button } from "@/components/ui/button";
...
export const Box = ({
  ...
  const rowsClasses = colsClasses; // reuse mapping for rows (1..12)
  ...
  const stackClasses = cn(
    ...
    getResponsiveClasses(gap, gapClasses),
    cols && getResponsiveClasses(cols, colsClasses),
    rows && getResponsiveClasses(rows, rowsClasses),
    className
  );
...
/* ---------- Hero (using the local Section/Container) ---------- */
