...
// TypeScript interface for Metric items
interface MetricItem {
  ...
  trendColor: string; // Base color for trend icon (e.g., '#E84045')
  trendStrokeColor: string; // Stroke color for trend icon path (e.g., '#F08083')
}
...
const TrendIndicatorIcon: React.FC<{
  direction: 'up' | 'down';
  baseColor: string; // e.g., '#E84045'
  strokeColor: string; // e.g., '#F08083'
...
const RAW_FUNNEL_DATA: FunnelDataPoint[] = [
  { key: 'Stage 1: Initial Alerts', data: 100 },
  { key: 'Stage 2: Acknowledged', data: 85 },
  { key: 'Stage 3: In Progress', data: 70 },
  { key: 'Stage 4: Resolved', data: 60 },
  { key: 'Stage 5: Closed', data: 50 },
  { key: 'Invalid Data', data: 'abc' as any },
];
...
const METRIC_ITEMS_DATA: MetricItem[] = [
  {
    id: 'mttr',
    icon: <MetricTagIcon />,
...
const StepInterpolationFunnelReportCard: React.FC = () => {
  const funnelAxisLineColor = 'rgba(126, 126, 143, 0.46)'; // #7E7E8F75 as rgba for better control if needed

  return (
    <div className="flex flex-col justify-between pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-[375px] h-[560px] overflow-hidden text-black dark:text-white transition-colors duration-300">
      <h3 className="text-3xl text-left p-7 pt-6 pb-8 font-bold">
        Incident Report
      </h3>

      <div className="h-[200px] w-full px-2 sm:px-4"> {/* Added padding for chart */}
        <FunnelChart
          id="interpolationFunnel"
          height={200}
          ...
          series={
            <FunnelSeries
              ...
              axis={
                <FunnelAxis
                  label={null} // Label is explicitly null as per source
      ...
      <div className="flex-grow flex flex-col pl-8 pr-8 font-mono divide-y divide-gray-200 dark:divide-[#262631] transition-colors duration-300 overflow-y-auto">
        {METRIC_ITEMS_DATA.map((item, index) => (
          <motion.div
            key={item.id}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: index * 0.05 }}
            className="flex w-full py-4 items-center gap-2" // Changed pb-4 pt-4 to py-4
          >
            ...
            <div className="flex gap-2 w-1/2 justify-end items-center">
              <span className="font-semibold text-xl text-black dark:text-white transition-colors duration-300">{item.value}</span>
              <TrendIndicatorIcon
...
export default StepInterpolationFunnelReportCard;

code.demo.1748121085681.tsx
import React from 'react';
...
```tsx
src/components/ui/interpolation-chart-1.tsx
'use client';
