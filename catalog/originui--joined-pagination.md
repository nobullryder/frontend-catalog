...
function JoinedPaginationDemo() {
  ...
  return (
    <div className="flex min-w-[300px] flex-col space-y-4 rounded-lg border p-4">
...
export { JoinedPaginationDemo };
```

Copy-paste these files for dependencies:
```tsx
/components/ui/joined-pagination.tsx
import { usePagination } from "@/components/hooks/use-pagination";
import { cn } from "@/lib/utils";
import { buttonVariants } from "@/components/ui/button";
import {
...
```bash
lucide-react, @radix-ui/react-slot, class-variance-authority, @radix-ui/react-icons
```
