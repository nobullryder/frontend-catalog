...
code.demo.1748119523251.tsx
import React from 'react';
import DetailedGroupedBarReport from '@/components/ui/multi-series-bar-big'; 

function DetailedGroupedBarReportDemoPage() {
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-800 p-4 transition-colors duration-300">
      <DetailedGroupedBarReport />
    </div>
  );
}

export default DetailedGroupedBarReportDemoPage;
```

Copy-paste these files for dependencies:
```tsx
src/components/ui/multi-series-bar-big.tsx
'use client';
