...
import {
  ...
  LinearYAxisTickSeries, // Added missing import
  StackedAreaSeries,
  Line,
  Area,
  Gradient,
...
const MAX_DAYS_DATA = 90;

const generateDailyData = (days: number): { key: Date; data: number }[] => {
  const data: { key: Date; data: number }[] = [];
  const today = new Date();
  for (let i = 0; i < days; i++) {
    const date = new Date(today);
    date.setDate(today.getDate() - (days - 1 - i)); // Oldest first, newest last
    date.setHours(0, 0, 0, 0);
    data.push({
      key: date,
      data: Math.floor(Math.random() * (50 - 5 + 1)) + 5, // Random data between 5 and 50
    });
  }
  return data;
};

const baseMultiDateDataRaw = [
  {
    key: 'Threat Intel',
    data: generateDailyData(MAX_DAYS_DATA),
  },
  {
    key: 'DLP',
    data: generateDailyData(MAX_DAYS_DATA),
...
// Validate data: ensure `data` properties are valid numbers
baseMultiDateDataRaw.forEach(series => {
  if (series.data) {
    series.data.forEach(item => {
      if (typeof item.data !== 'number' || isNaN(item.data)) {
        item.data = 0; // Default to 0 if invalid
      }
    });
  }
});

const baseMultiDateData = baseMultiDateDataRaw;

interface IncidentReportCardProps {}

function IncidentReportCard({}: IncidentReportCardProps): JSX.Element {
  ...
  const chartData = useMemo(() => {
    ...
    return baseMultiDateData.map(series => ({
      ...series,
      data: series.data.slice(-daysToDisplay),
    }));
  }, [timeRange]);
  ...
  return (
    <div className="flex flex-col justify-between pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] max-w-[600px] w-full h-[714px] overflow-hidden transition-colors duration-300">
      ...
      <div className="flex gap-8 w-full pl-8 pr-8 mb-4">
        <div className="flex gap-2 items-center">
          ...
          <span className="text-neutral-600 dark:text-[#9A9AAF] text-xs">Threat Intel</span>
        </div>
        <div className="flex gap-2 items-center">
          <div className="w-4 h-4 bg-[#EE4094]" />
          <span className="text-neutral-600 dark:text-[#9A9AAF] text-xs">DLP</span>
        </div>
        <div className="flex gap-2 items-center">
          <div className="w-4 h-4 bg-[#BB015A]" />
          <span className="text-neutral-600 dark:text-[#9A9AAF] text-xs">SysLog</span>
        </div>
      </div>
      
      <StackedAreaChart
        id="stacked-incident-chart"
        height={250} // Explicit height for chart
        data={chartData}
        xAxis={
          <LinearXAxis
            type="time"
              ...
              <LinearXAxisTickSeries
                ...
                label={
                  <LinearXAxisTickLabel
                    ...
                    fill="#9A9AAF" // Kept specific color, consider theming if needed
                  />
                }
              />
            }
            ...
            area={
              <Area
                ...
                gradient={
                  <Gradient
                    stops={[
                      <GradientStop key="gstop1" stopOpacity={0} />,
                      <GradientStop key="gstop2" offset="80%" stopOpacity={0.2} />,
                    ]}
                  />
                }
              />
            }
            colorScheme={['#FAE5F6', '#EE4094', '#BB015A']} // Specific chart colors
          />
        }
        gridlines={<GridlineSeries line={<Gridline strokeColor="#7E7E8F75" />} />} // Kept specific color
      />
...
code.demo.1748117547144.tsx
import React from 'react';
import DetailedStackedIncidentReportCard from '@/components/ui/area-chart-2'; // Adjust path as per your actual structure

function DetailedStackedIncidentReportCardDemoPage() {
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-800 p-4 transition-colors duration-300">
      ...
      */}
      <DetailedStackedIncidentReportCard />
    </div>
  );
}

export default DetailedStackedIncidentReportCardDemoPage;
```

Copy-paste these files for dependencies:
```tsx
src/components/ui/area-chart-2.tsx
'use client';
