...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  for (float i = 0.0; i < 80.0; i++)
  {
    ...
    // вращающаяся ось (зависит от z и времени)
    v = normalize(cos(t * 0.25 - z * 0.1 + vec3(0.0, 2.0, 4.0)));

    // поворот p вокруг v (проекция + cross)
    p = dot(v, p) * v + cross(v, p);

    // шаг реймарча (пульсирующая Z и “грязная” xy-норма)
    float zPulse = cos(p.z + t * 10.0) * 0.1;             // t/.1 -> t*10
    float xyComp = length(cos(p.xy)) - 0.6;
    d = 0.5 * length(vec2(zPulse, xyComp));

    z += d;

    // цветовой вклад (как в оригинале): … / d / z
...
code.demo.1756924173323.tsx
import  Component  from "@/components/ui/gigabit";
