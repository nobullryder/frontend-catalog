...
// Replace with your design system button if you prefer.
type SimpleButtonProps = React.ButtonHTMLAttributes<HTMLButtonElement> & {
  asChild?: boolean;
  variant?: "solid" | "link";
};
const SimpleButton = ({ className, variant = "solid", children, ...rest }: SimpleButtonProps) => {
  const base =
  ...
  const solid = "bg-primary text-primary-foreground hover:opacity-90 px-4 py-2";
  const link = "underline underline-offset-2 hover:opacity-80 px-0 py-0";
  return (
    <button className={cn(base, variant === "solid" ? solid : link, className)} {...rest}>
      {children}
    </button>
  );
};
      ...
      <Container className="grid items-stretch md:grid-cols-2 md:gap-12">
        ...
        <div className="flex flex-col gap-6 py-8">
          ...
          <div className="not-prose flex items-center gap-2">
            ...
            <Link href="#">
              <SimpleButton variant="link" className="w-fit">
                Learn More →
              </SimpleButton>
            </Link>
          </div>
        ...
        {/* Image side */}
        <div className="not-prose relative h-96 overflow-hidden rounded-lg border">
          <Image src={Placeholder} alt="placeholder" fill className="object-cover" priority />
        </div>
      </Container>
    </Section>
  );
