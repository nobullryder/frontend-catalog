...
code.demo.1748119304323.tsx
import React from 'react';
import GroupedBarIncidentReport from '@/components/ui/multi-series-bar'; 

function GroupedBarIncidentReportDemoPage() {
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-gray-800 p-4 transition-colors duration-300">
      <GroupedBarIncidentReport />
    </div>
  );
}

export default GroupedBarIncidentReportDemoPage;
```

Copy-paste these files for dependencies:
```tsx
src/components/ui/multi-series-bar.tsx
'use client';
