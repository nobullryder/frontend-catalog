...
const Component = React.forwardRef<HTMLDivElement, ComponentProps>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={`w-[90%]] h-[480px] group mx-auto dark:bg-[#252525] p-2 bg-white dark:border-0 border overflow-hidden rounded-md dark:text-white text-black ${className || ''}`}
      {...props}
    >
      <figure className='w-full h-80 group-hover:h-72 transition-all duration-300 dark:bg-[#0a121a] bg-[#f0f5fa] p-2 rounded-md relative overflow-hidden'>
        ...
        ></div>
        <img
          src={'https://i.postimg.cc/Prc7r9M4/Screenshot-2025-06-15-at-16-41-46.png'}
          alt='shoes'
          width={600}
          height={600}
...
code.demo.1749991448104.tsx
import React from 'react';
import Component from '@/components/ui/card-hover';

const ComponentDemo = () => {
  return (
    <div className='flex justify-center items-center min-h-[500px] p-4 bg-gray-100 dark:bg-gray-800'>
      <Component />
    </div>
  );
};
...
```tsx
src/components/ui/card-hover.tsx
import React from 'react';
import { ChevronRight } from 'lucide-react';
