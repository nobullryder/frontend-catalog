...
import {
  Heatmap,
  HeatmapSeries,
  HeatmapCellProps, // For typing if a custom cell was needed
  LinearYAxis,
...
// Placeholder icons, replace with actual SVG paths if different
const MetricTagIcon: React.FC<{ fill?: string }> = ({ fill = "currentColor" }) => (
...
const MetricClockIcon: React.FC<{ fill?: string }> = ({ fill = "currentColor" }) => (
 <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
...
const MetricAlertIcon: React.FC<{ fill?: string }> = ({ fill = "currentColor" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
...
const RAW_HEATMAP_DATA: HeatmapData = [
  ...
  { key: 'Wed', data: [{ key: '00h', data: 7 }, { key: '03h', data: 12 }, { key: '06h', data: 18 }, { key: '09h', data: 22 }, { key: '12h', data: 20 }, { key: '15h', data: 12 }, { key: '18h', data: null }, { key: '21h', data: 4 }] }, // Null data point
  { key: 'Thu', data: [{ key: '00h', data: 3 }, { key: '03h', data: 9 }, { key: '06h', data: 14 }, { key: '09h', data: 19 }, { key: '12h', data: 28 }, { key: '15h', data: 13 }, { key: '18h', data: 7 }, { key: '21h', data: 'invalid' as any }] }, // Invalid data point
  ...
  if (row.data && Array.isArray(row.data)) {
    row.data.forEach(item => {
      if (typeof item.data !== 'number' || isNaN(item.data)) {
        item.data = 0; // Default to 0 for null, undefined, or non-numeric
      }
    });
  } else {
    row.data = []; // Ensure data array exists
  }
});
const HEATMAP_DATA: HeatmapData = RAW_HEATMAP_DATA;

const HEATMAP_SERIES_COLOR_SCHEME = [
  { fill: '#FFD440', filter: 'drop-shadow(0px 0px 5px #FFD44090)' }, // Lightest
  { fill: '#F8A340' }, // Medium
  { fill: '#E84045' }  // Darkest
];
// For SequentialLegend, we typically provide an array of color strings.
// Reaviz might be able to infer this from the above, or we might need to simplify.
// Let's try with the Reaviz `schemes.unifyvizWarm` as in source.
...
const IncidentHeatmapReportCard: React.FC = () => {
  return (
    <div className="flex flex-col justify-between pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-[600px] h-[714px] overflow-hidden text-black dark:text-white transition-colors duration-300">
      ...
      {/* Heatmap and Legend Container */}
      <div className="flex w-full flex-grow px-2 min-h-0"> {/* flex-grow and min-h-0 for proper flex sizing */}
        <div className="flex-grow h-full min-w-0"> {/* min-w-0 for proper flex sizing of heatmap */}
          <Heatmap
            ...
            series={
              <HeatmapSeries
                colorScheme={HEATMAP_SERIES_COLOR_SCHEME as any} // Cast as any if Reaviz types don't directly support object array with filter
                padding={0.12}
              // cell={ // Example for custom cell if needed for advanced styling like filter
              //   <HeatmapCell
              //     style={(point: HeatmapCellProps) => ({
              //       filter: point.value > 20 ? 'drop-shadow(0px 0px 5px #FFD44090)' : undefined
              //     })}
              //   />
              // }
              />
      ...
      <div className="flex flex-col pl-8 pr-8 font-mono divide-y divide-gray-200 dark:divide-[#262631] transition-colors duration-300">
        {METRIC_LIST_ITEMS_DATA.map((item, index) => (
          <motion.div
            ...
            className="flex w-full py-3 items-center gap-2" // Adjusted padding
          >
...
```tsx
src/components/ui/heat-map-xl.tsx
'use client';
