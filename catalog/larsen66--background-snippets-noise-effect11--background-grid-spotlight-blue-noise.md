...
const Noise: React.FC<{ patternRefreshInterval?: number; patternAlpha?: number }> = ({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < d.length; i += 4) { const v = Math.random() * 255; d[i]=v; d[i+1]=v; d[i+2]=v; d[i+3]=patternAlpha; }
...
export default function Component() {
  return (
    <div className="fixed inset-0 -z-10 bg-slate-950">
      {/* SaaS Blue spotlight */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_620px_at_50%_200px,rgba(37,99,235,0.35),transparent_70%)]" />
      {/* Soft grid with top fade */}
