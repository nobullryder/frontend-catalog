...
{
  // базовая шейд-игрушечная раскладка
  ...
  {
    // цветовой вклад (как в исходнике)
    ...
    // поворот p вокруг v (проекция+векторное произведение)
    p = dot(v, p) * v + cross(v, p);

    // «скролл» по z
    p.z -= t;

    // клеточный модуль + свёртки
    vec3 q = abs(fract(p) - 0.5);             // q = |fract(p)-0.5|
    p = q + q.yzx - sin(z * 0.7);
...
code.demo.1756924079214.tsx
import Component  from "@/components/ui/geode";
