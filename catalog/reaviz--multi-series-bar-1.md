...
code.demo.1748119391017.tsx
import React from 'react';
import GroupedIncidentReportCard from '@/components/ui/multi-series-bar-1';

function GroupedIncidentReportCardDemoPage() {
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-neutral-900 p-4 transition-colors duration-300">
      <GroupedIncidentReportCard />
    </div>
  );
}

export default GroupedIncidentReportCardDemoPage;
```

Copy-paste these files for dependencies:
```tsx
src/components/ui/multi-series-bar-1.tsx
'use client';
