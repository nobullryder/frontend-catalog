...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    for (float i = 0.0, d = 0.0, z = 0.0; i++ < 3e1; o.rgb += (1.1 - sin(p)) / d)
    {
        ...
        // p.y = abs(mod((d = ++p.y) - 2., 4.) - 2.)
        d   = ++p.y;
        p.y = abs(mod(d - 2.0, 4.0) - 2.0);

        // p += .03 * sin(dot(cos(c), sin(c/.6 - t)) / .1) * (p.y - d)
        p  += 0.03 * sin(dot(cos(c), sin(c / 0.6 - t)) / 0.1) * (p.y - d);

        // z += d = .7 * length(vec3(cos(p.z/.1)*.1, p + sin(p.z*vec3(.7,1,0)+t)) - .1)
        // В GLSL ES явно берём .xy для (float, vec2) конструктора:
        z  += (d = 0.7 * length(
                  vec3(
                    cos(p.z / 0.1) * 0.1,
...
code.demo.1756890548809.tsx
import Component from "@/components/ui/fireflies-1";
...
```tsx
src/components/ui/fireflies-1.tsx
"use client";
import React, { useEffect, useRef } from "react";
