...
export function ComboBox() {
  ...
  const filteredOptions = top100Films.filter(film =>
    film.label.toLowerCase().includes(inputValue.toLowerCase())
  );

  const handleSelect = (film) => {
    setSelectedValue(film.label);
    setInputValue(film.label);
    setIsOpen(false);
  };
        ...
        {isOpen && (
          <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
            {filteredOptions.length > 0 ? (
              filteredOptions.map((film, index) => (
                <div
                  key={index}
                ...
                >
                  <div>
                    <div className="text-gray-900">{film.label}</div>
                    <div className="text-sm text-gray-500">{film.year}</div>
                  </div>
                  {selectedValue === film.label && (
